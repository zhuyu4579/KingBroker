...
//  GKCoverDemo
//
//  Created by 朱玉隆 on 16/8/24.
//  Copyright © 2016年 朱玉隆. All rights reserved.
...
static BOOL             _isTopStyleShow; //弹窗是否显示在上方
// 分离动画类型
...
//    bgView.fSize = CGSizeMake(KScreenW, KScreenH);
...
+ (void)showTopContentView
{
   
    [_fromView addSubview:_contentView];
        
    if (_animated) {
        NSInteger y = _contentView.fY;
        _contentView.fY = 0;
...
{
    if (_animated) {
        _contentView.fY = KScreenH;
        [UIView animateWithDuration:0.25 animations:^{
            _contentView.fY = KScreenH - _contentView.fHeight;
        }completion:^(BOOL finished) {
            !_showBlock ? : _showBlock();
        }];
    }else{
    ...
    switch (_showStyle) {
        case GKCoverShowStyleTop:  // 显示在顶部
        {
            if (_hideAnimStyle == GKCoverHideAnimStyleTop) {
                ...
                _contentView.fY = -_contentView.fY;
                [self remove];
            }
        }
            break;
