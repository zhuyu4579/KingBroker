...
@interface WZPreviewHouseController ()<UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,WZCyclePhotoViewClickActionDeleage,UIScrollViewDelegate>
//总view
...
@implementation WZPreviewHouseController
...
-(void)setFareHeights:(NSArray *)array{
    ...
    CGSize size = [_fare sizeThatFits:CGSizeMake(_fare.frame.size.width, CGFLOAT_MAX)];
    CGFloat n = size.height+60;
...
-(void)getUpScreen{
    //下滑view
    UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-JF_BOTTOM_SPACE)];
...
-(void)getUpTabButton{
    ...
    [popButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(buttonView.mas_left).offset(11);
...
-(void)getUpFive:(UIView *)view{
    ...
    UILabel *ScLabelOnes = [[EwenCopyLabel alloc] init];
    ...
    UILabel *ScLabelTwos = [[EwenCopyLabel alloc] init];
    ...
    UILabel *ScLabelThrees = [[EwenCopyLabel alloc] init];
