...
@interface WZVoucherDealController ()<HXPhotoViewDelegate>
...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"发起成交";
...
-(void)createController{
    ...
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:@"上传成交凭证" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang-SC-Medium" size: 15],NSForegroundColorAttributeName: [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0]}];
    ...
    NSMutableAttributedString *strings = [[NSMutableAttributedString alloc] initWithString:@"请上传认购协议书，pos机小票、收据/发票等成交凭证,审核通过后订单成交" attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFang-SC-Light" size: 12],NSForegroundColorAttributeName: [UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1.0]}];
    ...
    [boaringButton setTitle:@"发起成交" forState:UIControlStateNormal];
    ...
    [boaringButton addTarget:self action:@selector(voucherDeal) forControlEvents:UIControlEventTouchUpInside];
...
-(void)loadData{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/order/certificateDeal",HTTPURL];
