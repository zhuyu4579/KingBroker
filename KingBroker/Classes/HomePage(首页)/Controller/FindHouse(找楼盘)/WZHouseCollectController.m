...
@interface WZHouseCollectController ()<UICollectionViewDataSource,UICollectionViewDelegate>{
    //页数
    NSInteger current;
}
...
@implementation WZHouseCollectController
...
-(void)loadData{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/proProject/collectProList",HTTPURL];
...
-(void)cityDatas{
    ...
    //2.拼接参数
    NSMutableDictionary *paraments = [NSMutableDictionary dictionary];
    NSString *url =  [NSString stringWithFormat:@"%@/proProject/collectCityList",HTTPURL];
...
-(void)buttonCilck{
    ...
    if (_area.count == 0&&_buildingRenovation.count == 0&&_buildingFeature.count == 0&&_room.count == 0) {
        ...
        [but setTitleColor:UIColorRBG(255, 216, 0) forState:UIControlStateNormal];
        [but setImage:[UIImage imageNamed:@"lp_icon5"] forState:UIControlStateNormal];
    }
    [self hideView];
    _projectListArray = [NSMutableArray array];
