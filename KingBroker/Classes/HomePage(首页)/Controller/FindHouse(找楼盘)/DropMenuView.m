...
-(void)creatDropView:(UIView *)view withShowTableNum:(NSInteger)tableNum withData:(NSArray *)arr{
    ...
    if (!self.show) {
        ...
        CGFloat x = 0.f;
        CGFloat y = view.fY;
        CGFloat w = view.fWidth;
        CGFloat h = view.fHeight;
        
        self.frame = CGRectMake(x, y, w, h);
        self.cancelButton.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
        self.tableViewUnderView.frame = CGRectMake(0, 0, self.frame.size.width, self.fHeight);
...
-(void)adjustTableViews{
    ...
    for (int i = 0; i < addTableCount; i++) {
        ...
        adjustFrame.size.width = SCREEN_WIDTH / addTableCount ;
    ...
    if (idx == 0) {
        ...
        if (firstSelectRow == -1) {
            ...
            if (self.tableCount == 2) {
                ...
                return [self.dataArr[firstSelectRow][@"list"] count];
            }
        ...
        if (secondSelectRow == -1) {
            ...
            return [self.dataArr[firstSelectRow][@"list"][secondSelectRow][@"list"] count];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.tableCount == 1) {
        ...
        if (tableView == self.tableViewArr[0]) {
         ...
         NSInteger firstSelectRow = ((UITableView *)self.tableViewArr[0]).indexPathForSelectedRow.row;
         NSInteger secondSelectRow = ((UITableView *)self.tableViewArr[1]).indexPathForSelectedRow.row;
        cell.textLabel.textColor = UIColorRBG(102, 102, 102);
       // cell.textLabel.textAlignment = NSTextAlignmentCenter;
        if (tableView == self.tableViewArr[0]) {
            cell.textLabel.text = self.dataArr[indexPath.row][@"name"];
            cell.backgroundColor = UIColorRBG(242, 242, 242);
        }else if (tableView == self.tableViewArr[1]){
            cell.textLabel.text = self.dataArr[firstSelectRow][@"list"][indexPath.row][@"name"];
            cell.backgroundColor = UIColorRBG(248, 248, 248);
        }else if (tableView == self.tableViewArr[2]){
            
           cell.textLabel.text =  self.dataArr[firstSelectRow][@"list"][secondSelectRow][@"list"][indexPath.row][@"name"];
            cell.backgroundColor = UIColorRBG(255, 255, 255);
        }
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.tableCount == 1) {
        ...
        if (tableView == self.tableViewArr[0]) {
            ...
            NSString *provinceId = self.dataArr[indexPath.row][@"value"];
            _dicty[@"provinceId"] = provinceId;
            _dicty[@"cityId"] = @"";
            _dicty[@"areaId"] = @"";
            _dicty[@"areaName"] = self.dataArr[indexPath.row][@"name"];
            
            [_delegate dropMenuView:self didSelectName:_dicty];
        }else if (tableView == self.tableViewArr[1]){
            ...
            _dicty[@"cityId"] = self.dataArr[firstSelectRow][@"list"][indexPath.row][@"value"];
            _dicty[@"areaId"] = @"";
            NSString *name = self.dataArr[firstSelectRow][@"list"][indexPath.row][@"name"];
            if (![name isEqual:@"不限"]) {
                _dicty[@"areaName"] = name;
            }
            [_delegate dropMenuView:self didSelectName:_dicty];
        }else if (tableView == self.tableViewArr[2]){
            ...
            [self saveSelects];
            //[self dismiss];
            _dicty[@"areaId"] = self.dataArr[firstSelectRow][@"list"][secondSelectRow][@"list"][indexPath.row][@"value"];
            NSString *name  = self.dataArr[firstSelectRow][@"list"][secondSelectRow][@"list"][indexPath.row][@"name"];
            if (![name isEqual:@"不限"]) {
                _dicty[@"areaName"] = name;
            }
            [_delegate dropMenuView:self didSelectName:_dicty];
...
-(NSArray *)tableViewArr{
    ...
    if (_tableViewArr == nil) {
        UITableView *tableOne = [[UITableView alloc] init];
        tableOne.backgroundColor = UIColorRBG(245, 245, 245);
        UITableView *tableTwo = [[UITableView alloc] init];
        tableTwo.backgroundColor = UIColorRBG(248, 248, 248);
        UITableView *tableThree = [[UITableView alloc] init];
        tableThree.backgroundColor = UIColorRBG(255, 255, 255);
        _tableViewArr = @[tableOne,tableTwo,tableThree];
